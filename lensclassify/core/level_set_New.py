...
parser.add_argument('--lambda_3', type=float, default=0.0,
                    help='pretrain model parameters learning rate, eg pretrain_resnet50')
parser.add_argument('--e_ls', type=float, default=1.0/256.0,
                    help='pretrain model parameters learning rate, eg pretrain_resnet50')
...
if train_flag:
    for epoch in range(n_epochs):
        ...
        #	lr/=10
        #	optimizer = torch.optim.Adagrad(model.parameters(), lr=lr)
        #a = LevelSetModel.GetRNNParameters()
        optimizer = torch.optim.Adagrad(LevelSetModel.GetRNNParameters(),lr=lr)
