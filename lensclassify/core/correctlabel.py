...
def Matrix_to_array(data_path):
    ...
    l_x=np.arange(left+1,right,1)
    l_f=np.zeros(1,len(l_x))
    l_b=np.zeros(1,len(l_x))
    for i in range(col):
        A1=LensBoundary2[:,i+1]
        lo_A1=np.where(A1==1)
        if not lo_A1:
            break
        if len(lo_A1)>2:
            if i==0:
                ...
                disl_f=np.zeros(1,len(lo_A1))
                disl_b=np.zeros(1,len(lo_A1))
                for j in range(len(lo_A1)):
                    disl_f[j+1]=abs(lo_A1[j+1]-l_f[i]+top)
                    disl_b[j+1]=abs(lo_A1[j+1]-l_b[i]+top)
                locl_f=np.where(disl_f==np.min(disl_f))[0]
                locl_b=np.where(disl_b==np.min(disl_b))[0]
                l_f[i+1]=lo_A1[locl_f]+top
                l_b[i+1]=lo_A1[locl_b]+top
        else:
            l_f[i+1]=lo_A1[0]+top
            l_b[i+1]=lo_A1[-1]+top
    ...
    c_x=np.arange(left+1,right,1)
    c_f=np.zeros(1,len(c_x))
    c_b=np.zeros(1,len(c_x))
    for i in range(col):
        A1=LensBoundary[:,i+1]
        lo_A1=np.where(A1==1)
        if not lo_A1:
        ...
        if len(lo_A1)>2:
            if i==0:
                ...
                disc_f=np.zeros(1,len(lo_A1))
                disc_b=np.zeros(1,len(lo_A1))
                for j in range(len(lo_A1)):
                    disc_f[j+1]=abs(lo_A1[j+1]-c_f[i]+top)
                    disc_b[j+1]=abs(lo_A1[j+1]-c_b[i]+top)
                locc_f=np.where(disc_f==np.min(disc_f))[0]
                locc_b=np.where(disc_b==np.min(disc_b))[0]
                c_f[i+1]=lo_A1[locc_f]+top
                c_b[i+1]=lo_A1[locc_b]+top
        else:
            c_f[i+1]=lo_A1[0]+top
            c_b[i+1]=lo_A1[-1]+top
    ...
    for i in range(nleft-left,col+1):
    ...
    n_x=np.arange(nleft,nright,1)
    n_f=np.zeros(1,len(n_x))
    n_b=np.zeros(1,len(n_x))
    for i in range(nleft-left,nright-left+1):
        A1=NucleusBoundary[:,i]
        lo_A1=np.where(A1==1)
        if not lo_A1:
            continue
        if len(lo_A1)>2:
            if (i-nleft+left)==0:
                n_f[i-nleft+left+1]=lo_A1[0]+top
                n_b[i-nleft+left+1]=lo_A1[-1]+top
            else:
                disn_f=np.zeros(1,len(lo_A1))
                disn_b=np.zeros(1,len(lo_A1))
                for j in range(len(lo_A1)):
                    disn_f[j+1]=abs(lo_A1[j+1]-n_f[i-nleft+left]+top)
                    disn_b[j+1]=abs(lo_A1[j+1]-n_b[i-nleft+left]+top)
                locn_f=np.where(disn_f==np.min(disn_f))[0]
                locn_b=np.where(disn_b==np.min(disn_b))[0]
                n_f[i-nleft+left+1]=lo_A1[locn_f]+top
                n_b[i-nleft+left+1]=lo_A1[locn_b]+top
        else:
            n_f[i-nleft+left+1]=lo_A1[0]+top
            n_b[i-nleft+left+1]=lo_A1[-1]+top
...
def Delete_mistake_point(labeldictionary):
    ...
    for i in range(len(l_x)):
        if abs(l_f1[i+1]-l_f[i+1])>4:
            l_f[i+1]=0
        if abs(l_b1[i+1]-l_b[i+1])>4:
            l_b[i+1]=0
        if abs(c_f1[i+1]-c_f[i+1])>4:
            c_f[i+1]=0
        if abs(c_b1[i+1]-c_b[i+1])>4:
            c_b[i+1]=0
    ...
    for i in range(len(n_x)):
        if abs(n_f1[i+1]-n_f[i+1])>4:
            n_f[i+1]=0
        if abs(n_b1[i+1]-n_b[i+1])>4:
            n_b[i+1]=0
...
def Anew_fitting(labeldictionary):
    ...
    bdel=np.where(l_b==0)
    fdel=np.where(l_f==0)
    l_f=np.delete(l_f,fdel,1)
    l_b=np.delete(l_b,bdel,1)
    l_bx=np.delete(l_bx,bdel,1)
    l_fx=np.delete(l_fx,fdel,1)
    ...
    bdel=np.where(c_b==0)
    fdel=np.where(c_f==0)
    c_f=np.delete(c_f,fdel,1)
    c_b=np.delete(c_b,bdel,1)
    c_bx=np.delete(c_bx,bdel,1)
    c_fx=np.delete(c_fx,fdel,1)
    r3=np.polyfit(c_fx,c_f,4)
    A_cf_f=np.polyval(r3,A_cf_x)
    ...
    bdel=np.where(n_b==0)
    fdel=np.where(n_f==0)
    n_f=np.delete(n_f,fdel,1)
    n_b=np.delete(n_b,bdel,1)
    n_bx=np.delete(n_bx,bdel,1)
    n_fx=np.delete(n_fx,fdel,1)
    ...
    label={'A_lb_x':A_lb_x,'A_lb_y':A_lb_y,'A_lf_x':A_lf_x,'A_lf_y':A_lf_y,'A_cb_x':A_cb_x,'A_cb_y':A_cb_y,'A_cf_x':A_cf_x,'A_cf_y':A_cf_f,
