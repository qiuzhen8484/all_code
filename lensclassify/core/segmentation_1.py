...
def process_x_y(csv_data,idx,start,end,flag=False):
    ...
    x = x[mask]
    if flag:
        z1, x, y = new_ployfit(x, y, num=img.shape[1] - 1, start=start, end=end)
    else:
...
all_img_dir = './data/dataset7/eyes/'
...
for idx,path in enumerate(patient_idxs):
    ...
    for idx, img in enumerate(img_list):
        ...
        img_infor[img_name]['position'] = [ally[0],ally[1],BeginY]
        ...
        for color_id,csv_data in enumerate([[Lens1,Lens2],[Cortex1,Cortex2], [Nucleus1,Nucleus2]]):
            front,back = csv_data
            if color_id<2:
                _, front_x,front_y = process_x_y(front, idx, start=ally[0], end=ally[1])
                _, back_x, back_y = process_x_y(back, idx, start=ally[1], end=ally[0])
            else:
                z_1, front_x, front_y = process_x_y(front, idx, start=ally[0], end=ally[1], flag=True)
                z_2, back_x, back_y = process_x_y(back, idx, start=ally[1], end=ally[0], flag=True)
...
print 'success all the image information'
...
# filepath = './data/img'
# pathDir =  os.listdir(filepath)[:1]
#
# for allDir in pathDir:
#     savename = './data/result/' + allDir
#     child = os.path.join('%s/%s' % (filepath, allDir))
#     img = cv2.imread(child,0)
#     img = cv2.GaussianBlur(img, (3, 3), 0)
#     edges = cv2.Canny(img, 35, 110)
#     #edges = cv2.Canny(img, 90, 150)
#     # Sliding Window
...
#     #Draw The boundary
#     BeginY = imshape[1] / 3
#     newImage = img[BeginY:,ally[0]:ally[1]]
#     savename = './data/result/' + allDir
#     # cv2.imshow(savename,newImage)
#     green = (255, 255, 0)
#     cv2.line(newImage, (100, 100), (300, 300), green, 10)
...
#     #     point1 = (topy[i],topx[i])
#     #     point2 = (topy[i+1],topx[i+1])
#     #     cv2.line(img2, point1, point2, green, 2)
#
#     #savename = 'F:\\MyCode\\LensSegmentation\\result\\' + allDir
#     #cv2.imwrite(savename, img2)
#
#     # cv2.line(img2, (100,100), (800,800), green, 2)
