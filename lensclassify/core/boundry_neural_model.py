...
for epoch in range(args.epochs):
    ...
    for i, path in enumerate(img_list):
        ...
        tmp_boundry = segmentation.find_boundaries(tmp_gt)
        tmp_boundry = tmp_boundry.astype(np.uint8)
        tmp_boundry = np.transpose(tmp_boundry, [2, 0, 1])
        tmp_boundry = tmp_boundry[np.newaxis,:]

        # with open('./logs/test_data.pkl','w+') as f:
        #     pkl.dump(tmp_boundry,f)
        tmp_boundry = torch.from_numpy(tmp_boundry.astype(np.float32))
        tmp_boundry = Variable(tmp_boundry).cuda()
        model.zero_grad()
        if model_name=='HED':
            out, d1, d2, d3, d4, d5 = model(img)

            loss = bce2d(d1, tmp_boundry)
            loss += bce2d(d2, tmp_boundry)
            loss += bce2d(d3, tmp_boundry)
            loss += bce2d(d4, tmp_boundry)
            loss += bce2d(d5, tmp_boundry)
            loss += bce2d(out, tmp_boundry)
            print('epoch_batch: {:d}_{:d} | loss: {:f}'.format(epoch, i, loss.data[0]))
            ppi = out.cpu().data.numpy()
            ppi[ppi >= 0.5] = 1
            ppi[ppi < 0.5] = 0

        elif model_name=='BNM' or model_name=='Guanghui':
            out, boudry_out = model(img)
            out = torch.log(softmax_2d(out) + EPS)
            loss_1 = lossfunc(out, gt)
            # print boudry_out.size()
            # print tmp_boundry.size()
            loss_2 = bce2d(boudry_out, tmp_boundry)
            loss = loss_1 + 100*loss_2
            # print('epoch_batch: {:d}_{:d} | loss_1: {:f}| loss_2: {:f}'.format(epoch, i, loss_1.data[0], loss_2.data[0]))
            ppi = np.argmax(out.cpu().data.numpy(), 1).reshape((args.img_size, args.img_size))
        elif 'BNM' in model_name:
            out = model(img)
            # print out[0].sizes()
            # print tmp_boundry.size()
            loss = bce2d(out[0], tmp_boundry)
