...
models_list = ['UNet128',  'UNet256',      'UNet512',  'UNet1024',    'UNet512_SideOutput', 'UNet1024_SideOutput',
              'resnet_50', 'resnet_dense', 'PSP',      'dense_net',    'UNet128_deconv',     'UNet1024_deconv',
               'FPN18',    'FPN_deconv',   'Unet',     'Multi_scale',  'ASPP']
...
parser.add_argument('--best_model', type=str,  default='70_1e-05.pth',
...
if args.pre_all:
    ...
    # model_path = os.path.join('models', 'multi_scale_' + model_name, args.best_model)
    print model_path
...
for idx, img_path in enumerate(test_img_list):
    ...
    if model_name == 'Multi_scale':
        out = model.MutiScaleForward(resize_img)
        ...
        out = model.ASPPforward(resize_img)
    ...
    if save_test_img:
        ...
        # if not os.path.exists(os.path.join(args.results,'multi_scale_'+model_name)):
        #     os.mkdir(os.path.join(args.results,'multi_scale_'+model_name))
        # save_name = os.path.join(args.results,'multi_scale_'+model_name, img_path)
    ...
    if MSE_loss[0]>10 and args.flag=='train':
