...
def Anew_fitting(labeldictionary, save_path):
    l_x=labeldictionary['l_x']
    l_f=labeldictionary['l_f']
    l_b=labeldictionary['l_b']
    c_x=labeldictionary['c_x']
    ...
    save_name = os.path.join(save_path, os.path.basename(labeldictionary['datapath']))
    ...
    scio.savemat(save_name, label)

def Get_struct_label(data_path, save_path, five_point_fit_flag=False):
    img_name = []
    for root, dirs, files in os.walk(data_path):
        for file in files:
            img_name.append(os.path.join(data_path, file))
    if five_point_fit_flag is True:
        for i in range(len(img_name)):
            Anew_fitting(Delete_mistake_point(Matrix_to_array(img_name[i])), save_path)
    else:
        for i in range(len(img_name)):
            Anew_fitting(Matrix_to_array(img_name[i]), save_path)
